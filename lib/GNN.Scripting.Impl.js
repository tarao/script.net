...
package GNN.Scripting.Reflection {
    class Assembly {
        ...
        function load() : Assembly {
            ...
            var man : Manager = (o.domain == 'separate') ?
    ...
    class AssemblyLoader implements PreLoader {
        function load(fname : String, options) : String {
            ...
            var file : String = cache.asm.Location;
            cache.unload();
            return file;
        }
    }
        ...
        function compile(source : Source) : void {
            ...
            try {
                if (!preprocessor) {
                    ...
                    context.loader = new AssemblyLoader();
        ...
        function getTempFiles() : Hashtable {
            if (this.compiled) {
                ...
                if (files) {
                    ...
                    for (var i : int = 0; !e.atEnd(); e.moveNext()) {
                        list[i++] = e.item();
                    }
                    hash.Add('files', list);
                    return hash;
