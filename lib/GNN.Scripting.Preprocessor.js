...
package GNN.Scripting.Preprocessor {
    ...
    class Import extends Base {
        ...
        function line(l : Line) : Code {
            ...
            if (m && (m[3] || m[4])) {
                ...
                file = maybeCompile(file);
                this.context.importer.importLocalAssembly(file);
            }
            return l;
        }

        function maybeCompile(file : String) : String {
            if (/\.(?:js|cs)$/.test(file) && this.context.loader) {
                var options = { target: 'library', cache: true };
                options.lang = file.substring(file.length-2);
                try {
                    return this.context.loader.load(file, options);
                } catch (err) {
    ...
    class ImportOnce extends Base {
        function ImportOnce(context : Context) {
            super(context);
        ...
        function line(l : Line) : Code {
            ...
            var m : RegExpMatch = regex.exec(l.code);
            if (m) {
                if (this.packages.Contains(m[2])) {
                    l.code = '';
                } else {
                    this.packages.Add(m[2], true);
                }
            }
            return l;
        }

        var packages : IDictionary = null;
    }
    ...
    interface PreLoader {
        function load(fname : String, options) : String;
    }
