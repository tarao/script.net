...
package GNN {
    ...
    class ScriptRunnerImpl extends MarshalByRefObject {
        var options = {};
        ...
        function run(fname : String, ...args : String[]) {
            ...
            // compile
            var compiled = script.compile(fname, provider);
            this.files = compiled.result.TempFiles;
            if (0 < compiled.result.Errors.Count) {
                ...
                try {
                    ...
                    compiled.reportError(e, Console.Error);
                }
            }
        }
    }
    ...
    class ScriptRunner {
        ...
        function run(fname : String, ...args : String[]) {
            this.runner.run.apply(this.runner, [ fname ].concat(args));
        }

        function clean() {
            var base = this.runner.files.BasePath;
